...
void Print::process()
{
    ...
    m_sequential_collision_detected = config().complete_objects ? check_seq_conflict(*this, config()) : std::nullopt;
...
std::string Print::export_gcode(const std::string& path_template, GCodeProcessorResult* result, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if (result && m_sequential_collision_detected && result->moves.empty())
